...
if (is_file($installFile)) {
    ...
    switch ($step) {
        ...
        if (!is_writeable($indexFile)) {
            $error = TRUE;
            echo "<div class='alert alert-error'><i class='icon-remove'></i> Index Filer (index.php) is not write able!</div>"; 
        }
            ...
            if ($object->status == 'success') {
                if(isset($object->database)) {
                    ...
                    if ($database->update_db($dbdata, $_POST['username'], $_POST['code']) == false) {
                        ...
                        $finished = TRUE;
                        if(!@unlink('../SMA_POS')){
                            echo "<div class='alert alert-warning'><i class='icon-warning'></i> Please remove the SMA_POS file from the main folder in order to lock the update tool.</div>";
                        }
                    }
